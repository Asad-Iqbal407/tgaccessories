  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const filtered = (data.products || []).filter((p: Product) => p.category === params.id);
  ...
  return (
    <div className="bg-gray-50">
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="mb-8">
          <h2 className="text-4xl font-bold text-gray-900 mb-4">{params.id.replace('-', ' ').replace(/^\w/, c => c.toUpperCase())}</h2>
          <p className="text-xl text-gray-600">Browse products in {params.id}</p>
        </div>
